(function pricingBoot() {
  ...
  if (window.location.protocol === 'file:') {
    showError(
      ...
      'Start a local server from this folder, then open http://localhost:8081/pricing.html\n\n' +
  ...
  status.textContent = 'Loading pricing page…';
  document.body.appendChild(status);

  const componentScripts = [
    'scripts/raw-component-1-fd348cec.js',
    'scripts/raw-component-5-pricing.js',
  ];
  ...
  (async function run() {
    try {
      ...
      if (typeof PricingPage !== 'function') {
        throw new Error('PricingPage not found after loading components');
      }

      status.remove();
      ReactDOM.createRoot(rootEl).render(React.createElement(PricingPage));
    } catch (err) {
      status.remove();
